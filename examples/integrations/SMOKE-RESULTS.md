...
**Date:** 2026-07-24 (matrix); **honesty update:** 2026-09-09 — Google ADK / Gemini marked **SKIP** (not FAIL) when keys are absent.  
...
**Status legend:** **PASS** = live success · **SKIP** = required keys/packages absent (not a product failure; re-run after setting keys) · **FAIL** = keys/packages present but the call errored.
...
| Google ADK | `google_adk_galileo.py` | **SKIP** | — | No | Missing `GOOGLE_API_KEY` / `GEMINI_API_KEY` (not FAIL). Packages installed (`galileo-adk`, `google-adk`). Starter prints `SMOKE SKIP` and exits 0. **Do not invent keys.** |
| Gemini / Vertex Enterprise | `gemini_enterprise_galileo.py` | **SKIP** | — | No | Missing `GOOGLE_API_KEY` **or** `GOOGLE_APPLICATION_CREDENTIALS` + `VERTEX_PROJECT` (not FAIL). Starter prints `SMOKE SKIP` and exits 0. |
...
6. **`google_adk_galileo.py` / `gemini_enterprise_galileo.py`** — print `SMOKE SKIP` (exit 0) when Google keys absent; `SMOKE FAIL` only when keys present but the call errors.

## Blockers needing user action

| Blocker | Needed |
|---------|--------|
| Google ADK + Gemini/Vertex | Set `GOOGLE_API_KEY` (or `GEMINI_API_KEY`), **or** Vertex: `GOOGLE_APPLICATION_CREDENTIALS` + `VERTEX_PROJECT` (+ optional `VERTEX_LOCATION`). Re-run those two starters — expect `SMOKE PASS` (or `SMOKE FAIL` if the live call errors). Do not invent keys. |
...
# After Google keys (expect SMOKE PASS / FAIL — not silent):
...
# Without Google keys the same commands print SMOKE SKIP and exit 0.
```

...
# ── Real engineering knowledge corpus (ML training / inference / infra) ────────
...
def load_kb(poisoned: bool = False) -> list:
    if poisoned and KB_POISON_FILE.exists():
        with open(KB_POISON_FILE) as f:
            return json.load(f)
    if KB_FILE.exists():
        with open(KB_FILE) as f:
            return json.load(f)
    return KNOWLEDGE_BASE_POISONED if poisoned else KNOWLEDGE_BASE_ORIGINAL
...
if __name__ == "__main__":
    ...
    if "--poison-corpus" in args:
        ...
        save_kb(KNOWLEDGE_BASE_ORIGINAL, KB_FILE.with_suffix(".json.bak"))
    ...
    if "--restore-corpus" in args:
        ...
        print("✅ Corpus restored to the real engineering knowledge base.")
        sys.exit(0)
    ...
    if "--batch" in args or "--baseline" in args:
        ...
        print("Usage: python app.py 'Your question'  |  --batch  |  --poison-corpus  |  --restore-corpus")
